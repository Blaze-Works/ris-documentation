...
(function (global, factory) {
    ...
    let drawBox = function (color, parent) {
        ...
        if (parent) { addChild(parent, [box]) }
        else if (!parent) { addChild('body', [box]) }
        else if (!color && !parent) { box = elemt('div', null, 'box', null, 'height: 30px;width: 30px;background-color: #000;float: left'); addChild('body', [box]) }
        return box;
    };
    ...
    let input = function (place, data) {
        ...
        if (data.input) {
            ...
            for (let i in data.input) {
                ...
                addChild(this.inputWrapper, [this.inputContainer])
            }
        } else {
            this.inputWrapper = null;
        }
        addChild(place, [this.inputWrapper]);
    ...
    let CanvasRenderer = function () {
        ...
        this.imageData = function (x = 0, y = 0, w = _canvasWidth, h = _canvasHeight, settings) {
            return this.ctx.getImageData(x, y, w, h, settings);
        };

        this.putImageData = function (imageData, dx = 0, dy = 0) {
